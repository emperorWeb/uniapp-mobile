...
export default{
	data() {
		return{
			...
			name: '',
			charts: '',
			opinionData: ["100", "300", "211", "321","233","150","210"],
			attributeData:["第一周","第二周","第三周","第四周","周五","周六","周日"],
			timeData:1,
			typeData:11
		}
	},
	...
	methods:{
		...
		select(item, index) {
			...
			uni.request({
				url:'http://122.114.120.6:3000/identity/administrator/accordingTime',
				header: {'content-type': 'application/x-www-form-urlencoded', },
				data:{nameId:nameId,typeData:this.typeData,timeData:this.timeData},
				method:'POST',
				dataType:'json',
				success:res =>{
		...
		ferver(item, index) {
			...
			uni.request({
				url:'http://122.114.120.6:3000/identity/administrator/accordingType',
				header: {'content-type': 'application/x-www-form-urlencoded', },
				data:{
					nameId:nameId,
