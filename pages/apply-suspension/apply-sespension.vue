	...
	<view class="registration">
		<view>
			...
			<view class="identify-icon">
				...
				<view class="identify-icon-text">{{text}}</view>
			</view>
		</view>
		<!-- 登记区域 -->
...
export default{
	...
	onLoad:function(options){
		this.appli = JSON.parse(options.appli); 
		if(this.appli == 1){
			this.text = '休学申请'
		}else if(this.appli == 2){
			this.text = '转学申请'
		}else if(this.appli == 3){
			this.text = '退学申请'
		}else if(this.appli == 4){
			this.text = '复学申请'
		}
	},
	methods:{
		...
		chooseImages:async function(){
		    ...
		    uni.chooseImage({
		        ...
		        success:(res)=> {
					...
					// .then(base64 => {
					// 	this.imageBase = this.imageBase.concat(base64);
					//  })
					//  .catch(error => {
					// 	console.error(error);
					//  });
		...
		btnVisit(){
			...
			uni.request({
				url:'http://122.114.120.6:3000/identity/student/suspension',
				header: {
				...
				success:res =>{
					...
					if(res.data.code == 400){
						uni.showToast({title: '学号不存在',icon: 'none'});
					}else if(res.data.code == 100){
						uni.uploadFile({
							url:'http://122.114.120.6:3000/identity/student/image?username='+this.username+'&cardId='+this.cardId+'&why='+this.why+'&appli='+this.appli,    
